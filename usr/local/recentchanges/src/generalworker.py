...
class GeneralWorker(Worker):
    ...
    def run_op(self, action):
        try:
            ...
            try:
                if action == "benchmark":
                    """ not currently used as needs root see execute_benchmark main.py """
                    rlt = perform_read_test(self.lclhome, self.basedir, self.usr, self.exclDIRS, log_fn=self.log.emit)
